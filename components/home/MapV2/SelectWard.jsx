...
export default function SelectWard({ className, data, wardId, setWardId, handleChangeWard, titleWard, setTitleWard }) {
            ...
            {isOpen && (
                <div className='absolute bottom-0 left-0 translate-y-[110%] bg-white rounded-[0.5vw]'>
                    {data?.map((e, index) => (
                        <div
                            className={`${
                                wardId === Number(e?.ward_id) ? 'bg-gray-300 cursor-default' : 'cursor-pointer'
                            } whitespace-nowrap`}
                            onClick={() => {
